...
const BOARDS = [
  ...
  { title: 'Goals per game', min: QUAL,
    note: 'Goals scored, per appearance.',
    value: p => num(p.goals) / num(p.apps),
    fmt: (v, p) => `${v.toFixed(2)} <span class="muted small">(${n1(p.goals)})</span>` },
  { title: 'Assists per game', min: QUAL,
    note: 'Assists provided, per appearance.',
    value: p => num(p.assists) / num(p.apps),
    fmt: (v, p) => `${v.toFixed(2)} <span class="muted small">(${n1(p.assists)})</span>` },
  { title: 'Goal involvements per game', min: QUAL,
    cardNote: 'Goals + assists ÷ games',
    note: 'Goals and assists combined, per appearance.',
    value: p => (num(p.goals) + num(p.assists)) / num(p.apps),
    fmt: (v, p) => `${v.toFixed(2)} <span class="muted small">(${n1(num(p.goals) + num(p.assists))})</span>` },
  { title: 'Highest win rate', min: QUAL,
    cardNote: 'Share of games won',
    note: 'Share of games won (games with no recorded result are excluded).',
    value: p => num(p.win_pct),
    fmt: (v, p) => `${v.toFixed(0)}% <span class="muted small">(${p.w}W)</span>` },
